...
/**
 * Class DeployAdminCommand
 * @package Kega\DeployDedicated\Console\Command
 */
class DeployAdminCommand extends Command
{
    /**
     * @var Deploy
     */
    ...
    {
        $this->setName('update:admin')
             ->setDescription('Deploy the admin server.')
             ->setHelp('This command pulls updates from git and does all things necessary to get the new code running on' .
                ' the server that is running the database.')
             ->addOption(\Kega\DeployDedicated\Helper\Deploy::OPT_NO_DISABLE_MAINTENANCE, null, InputOption::VALUE_NONE,
                'Leave maintenance mode on afterwards')
             ->addOption(\Kega\DeployDedicated\Helper\Deploy::OPT_NO_GIT, null, InputOption::VALUE_NONE, 'Skip git updates')
             ->addOption(\Kega\DeployDedicated\Helper\Deploy::OPT_NO_UPGRADE, null, InputOption::VALUE_NONE,
                'Skip bin/magento setup:upgrade');
        parent::configure();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            $this->helper->setupUpgrade();
            $this->helper->compileDi();
            $this->helper->flushStaticContent();
            $this->helper->deployStaticContent();
